Spree::BaseHelper.class_eval do
  ...
  def get_weekly_drawing_link(sweepstake)
    ...
    link = content_tag :a, :href => "#", :class => "btn btn-primary", :onclick => "weeklyPick('/', '#{get_name(sweepstake)}', '#{sweepstake.description}', '#{get_url(sweepstake)}', '#{get_image(sweepstake)}', '#{sweepstake.id}');return false;" do
